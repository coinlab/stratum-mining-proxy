...
MAX_TARGET = float(0xFFFF << (256 - 48))
...
class JobRegistry(object):
    ...
    def getwork(self, no_midstate=True):
        ...
        #        'hash1': hash1}
        # only return block header so bill's code doesn't crash
        result = {'data': block_header}
    ...
    def submit(self, header, worker_name):
        ...
        if utils.uint256_from_str(hash_bin) > self.target:
            gen_message(self.target, hash_bin, ntime, nonce, error="Share is below expected target")
        ...
        try:
            ...
            gen_message(self.target, hash_bin, ntime, nonce, error="Job not found")
            #log.info("Job not found")
            return False

        gen_message(self.target, hash_bin, ntime, nonce)
